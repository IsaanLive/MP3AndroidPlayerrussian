...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Всего загрузок: "+totaldownloading);
        }else{
        	text.setText("Всего загрузок: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Завершенных и неудавшихся загрузок");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Ожидается загрузок");
		    		}else if(a == 0){
		    			progress.setText("Завершенных загрузок");
		    		}else if(a == -1){
		        		progress.setText("Неудавшихся загрузок");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("скачать вариант")
				       .setCancelable(false)
				       .setNegativeButton("Удалить", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("Загрузка MP3 удалена");
				           }
				       })
				...
				final CharSequence[] items = {"Открыть MP3", "Установить в качестве мелодии звонка", "Повторить загрузку", "Удалить", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Выбрать вариант");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Открыть MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Ошибка загрузки MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Установить в качестве мелодии звонка"){
							...
							builder.setMessage("Вы уверены, что хотите удалить: " + title + " ?")
							       ...
							       .setPositiveButton("Да", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 удален");
							           }
							       })
							       .setNegativeButton("Нет", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 не удален");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Повторить загрузку"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Повторить");
				    	}
				...
				builder.setMessage("скачать вариант")
				       .setCancelable(false)
				       .setNegativeButton("удаление", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("Загрузка MP3 удалена");
				...
				builder.setMessage("скачать вариант")
				       .setCancelable(false)
				       .setNegativeButton("Остановить загрузку", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("Загрузка MP3 остановлена");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Сейчас загружается", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Ожидается загрузка", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Завершенных загрузок", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Неудавшихся загрузок", 100));
