...
public class SearchActivity extends ListActivity {
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("Поиск был отменен");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("Поиск композиций ...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Похоже, у вас отсутствует активное подключение к Интернету или же у вас слабый сигнал. Пожалуйста, включите Wi-Fi, введите данные и попробуйте еще раз.");
		        }else{
					failresults.setText("Результаты не найдены");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Кажется, при попытке поиска произошла ошибка. Пожалуйста, попробуйте еще раз");
		        }
			}else{
				setListAdapter(adapter);
			}
		...
		public void getDilandau(String search) throws IOException, MalformedURLException, NullPointerException{
			...
			if(!sb.toString().contains("Песни не найдены")){
